...
export const useSmartAlerts = () => {
  ...
  const loadAlerts = useCallback(() => {
    try {
      const stored = smartAlertsStorage.getAlerts();
      setAlerts(stored);
  ...
  const addAlert = useCallback((alert: SmartAlert) => {
    smartAlertsStorage.addAlert(alert);
    setAlerts(prev => [...prev, alert]);
  }, []);

  const removeAlert = useCallback((id: string) => {
    smartAlertsStorage.removeAlert(id);
    setAlerts(prev => prev.filter(a => a.id !== id));
  }, []);

  const refreshPrices = useCallback(() => {
    const currentAlerts = smartAlertsStorage.getAlerts();
    ...
    for (const alert of updated) {
      smartAlertsStorage.updateAlert(alert.id, alert);
    }
    setAlerts(updated);
